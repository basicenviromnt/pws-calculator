...
def load_data():
    try:
        ...
        # Применяем .str.strip() ко всем текстовым колонкам
        ...
        data['params'] = {
            ...
            "windowsill_cap_price": 90, # Цена заглушки
            "windowsill_master_commission_pm": 450, # Комиссия
            "drip_markup_pm": 500,
        ...
        st.error(f"ОШИБКА: Не найден файл CSV: {e.filename}. Для этого теста нужен ТОЛЬКО 'price_main.csv' в той же папке.")
        return None
...
def calculate_windowsill_price(params, data):
    ...
    try:
        ...
        # 1. Находим товар
        # ИСПРАВЛЕНО: Используем 'str.contains' для надежности
        product_info = df_pricelist[
            ...
            (str(df_pricelist['Валюта']) == str(params['width_mm'])) # Валюта = Ширина (мм)
        ].iloc[0]
...
if data:
    ...
    try:
        if service_key == "windowsill":
            ...
            # --- НОВАЯ ПРОВЕРКА ДЛЯ ОТЛАДКИ ---
            if df_options.empty:
                st.error("ОШИБКА: Не найдено ни одной строки со словом 'Підвіконня' в колонке 'Виріб' вашего файла 'price_main.csv'.")
                st.info("Пожалуйста, убедитесь, что вы загрузили на GitHub 'чистый' файл, который мы проверяли ('Новая таблица - Лист1 (2).csv') и переименовали его в 'price_main.csv'.")
                ...
                # 'Валюта' -> Ширина (мм)
                # Преобразуем в str, чтобы избежать ошибок типов
                df_options_width = df_options_color[
                    (df_options_color['Вартість'] == params['color'])
                ]
                # Конвертируем 'Валюта' (где у нас ширина) в строки
                width_options = df_options_width['Валюта'].astype(str).unique()
